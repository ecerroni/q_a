...
t.test(
  "should be allowed to delete the family from the family array of the member if the family is soft deleted",
  async t => {
    ...
    data.forEach(function(item) {
      if (item.collection === "Member" && item.deletion === "weak") {
        ...
        t.equal(
          !!item.entityIdFields.find(item => item.weakIds.includes(familyId)),
          true,
        );
      }
...
t.test(
  "should be allowed to remove user from course when user is deleted",
  async t => {
    const currentUserId = users.filter(user => user.username === "veronica")[0]
      ._id;
    const [err, res] = await to(
      test(
        JSON.stringify({
    ...
    data.forEach(function(item) {
      ...
      if (item.collection === "User" && item.deletion === "soft") {
        t.equal(item.ids.length > 0, true);
        t.equal(item.ids.includes(currentUserId), true);
      }
      const deletedItems = Object.values(item.items).reduce((arr, array) => {
        return [
          ...arr,
