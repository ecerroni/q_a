...
export default async db => {
  ...
  const users = await mockedUsers.reduce(async (arr, user) => {
    ...
    const userMachine = machines.filter(
      machine => machine.owner === user.username,
    );
    if (userMachine.length) {
      await db.models.Machine.insertMany([
        ...userMachine.map(ur => ({ ...ur, owner: updatedUser._id })),
      ]);
    }
