    ...
    visitor: {
      ImportDeclaration: function ImportDeclaration(path, state) {
        ...
        if (!source.match(/^[./]/)) {
          ...
          source =
            anyModulePath.replace(/(.*\/node_modules\/).*/, "$1") + source;
        } else {
        ...
        node.specifiers.forEach(function(_) {
          if (t.isImportNamespaceSpecifier(_)) {
            lookFor.push({ source: "*", target: _.local.name });
          } else if (t.isImportSpecifier(_)) {
            lookFor.push({ source: _.imported.name, target: _.local.name });
          }
        });

        // get files
        var files = (_glob.sync || _glob.default.sync)(source, { dot: true });
        if (!files.length) {
        ...
        if (lookFor.length) {
          lookFor.forEach(function(importedFile) {
            if (importedFile.source === "*") {
              ...
              var func = t.functionDeclaration(
                ...
                t.blockStatement([
                  t.variableDeclaration("var", [
                    t.variableDeclarator(t.identifier("o"), t.identifier("v")),
                  ]),
              ...
              files.forEach(function(fileName) {
                ...
                var relative =
                  "./" +
                  _path2.default.relative(
                    _path2.default.dirname(workingDirectory),
                    fileName,
                  );
              ...
              var definition = t.variableDeclaration("var", [
                t.variableDeclarator(t.identifier(importedFile.target)),
              ]);
              ...
              files.forEach(function(fileName) {
                ...
                var thing = t.assignmentExpression(
                  "=",
                  t.identifier(importedFile.target),
                  id,
                );
          ...
          throw path.buildCodeFrameError(
            "Syntax error: wrong import statement",
          );
        }
      },
    },
