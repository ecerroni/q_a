...
import { throwIfError, log } from "../../../utils";

const getDeleteArray = async (relations, dl) => {
        ...
        const ids = await tie.foreignKeys.reduce(async (a, fk) => {
          ...
          if (fk.isParentEntityId) {
            ...
            if (tie.connection === "dirty") {
              ...
              items = await dl[fk.entity].loadManyByQuery({
            ...
            if (tie.connection === "dirty") {
              ...
              // let weakIds = await dl[fk.entity][fk.entity].find({_id: { $in: validIds }})
...
const buildDeleteArray = async (entities, dl) => {
  ...
  const deleteArray = await getDeleteArray(collections, dl);
...
export default class extends BaseModelDataSource {
  ...
  async delete(entities, dry = false) {
    ...
    try {
      ...
      array = await buildDeleteArray(array, this.context.dataLoaders);
      ...
      if (!isTesting) {
        log.dev(JSON.stringify({ "Deletion Array": array }, null, 2));
      }
      if (dry) return null;
      // perform deletions
